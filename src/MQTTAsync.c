...
void MQTTAsync_checkDisconnect(MQTTAsync handle, MQTTAsync_command* command)
{
	...
	if (m->c->outboundMsgs->count == 0 || MQTTAsync_elapsed(command->start_time) >= command->details.dis.timeout)
	{
		...
		if (command->details.dis.internal)
		{
			...
			{
				Log(TRACE_MAX, -1, "Calling connectionLost for client %s", m->c->clientID);
				(*(m->cl))(m->context, NULL);
			}
		...
		{
			Log(TRACE_MAX, -1, "Calling disconnect complete for client %s", m->c->clientID);
	...
	if ((found = ListFindItem(handles, &socket, clientSockCompare)) != NULL)
	{
		...
		if (m->pending_write)
		{
			...
			if (cur_response && command->onSuccess)
			{
				...
				Log(TRACE_MAX, -1, "Calling publish success for client %s", m->c->clientID);
	...
	*/
	...
	if (command->command.type == CONNECT)
	{
		...
		else
		{
			...
			Log(TRACE_MAX, -1, "Connecting to serverURI %s with MQTT version %d", serverURI, command->command.details.conn.MQTTVersion);
#if defined(OPENSSL)
		...
		if (command->command.details.pub.qos == 0)
		{
			if (rc == TCPSOCKET_COMPLETE)
			{
				if (command->command.onSuccess)
				{
					...
					Log(TRACE_MAX, -1, "Calling publish success for client %s", command->client->c->clientID);
	...
	else if (rc == SOCKET_ERROR || rc == MQTTASYNC_PERSISTENCE_ERROR)
	{
		...
		{
			Log(TRACE_MAX, -1, "Connect failed, more to try");
		...
		else
		{
			...
			{
				Log(TRACE_MAX, -1, "Calling command failure for client %s", command->client->c->clientID);
	...
	while (ListNextElement(handles, &current))		/* for each client */
	{
		...
		if (m->c->connect_state != 0 && MQTTAsync_elapsed(m->connect.start_time) > (m->connectTimeout * 1000))
		{
			if (MQTTAsync_checkConn(&m->connect, m))
			{
				...
				conn->client = m;
				conn->command = m->connect;
				Log(TRACE_MAX, -1, "Connect failed with timeout, more to try");
				MQTTAsync_addCommand(conn, sizeof(m->connect));
			}
			else
			{
				...
				if (m->connect.onFailure)
				{
					...
					Log(TRACE_MAX, -1, "Calling connect failure for client %s", m->c->clientID);
		...
		while (ListNextElement(m->responses, &cur_response))
		{
			...
			else
			{
				...
				{
					Log(TRACE_MAX, -1, "Calling %s failure for client %s",
		...
		if (m->automaticReconnect && m->retrying)
		{
			if (m->reconnectNow || MQTTAsync_elapsed(m->lastConnectionFailedTime) > (m->currentInterval * 1000))
			{
				...
				if (m->c->MQTTVersion == MQTTVERSION_DEFAULT)
					conn->command.details.conn.MQTTVersion = 0;
				Log(TRACE_MAX, -1, "Automatically attempting to reconnect");
				MQTTAsync_addCommand(conn, sizeof(m->connect));
				m->reconnectNow = 0;
			}
		}
...
void MQTTAsync_destroy(MQTTAsync* handle)
{
	...
	if (m->c)
	{
		...
		if (!ListRemove(bstate->clients, m->c))
			...
			Log(TRACE_MAX, 1, NULL, saved_clientid, saved_socket);
		free(saved_clientid);
	}
		...
		else
		{
			if (m->c->messageQueue->count > 0)
			{
				...
				if (rc)
				{
					...
					Log(TRACE_MAX, -1, "False returned from messageArrived for client %s, message remains on queue",
						m->c->clientID);
			}
			if (pack)
			{
				if (pack->header.bits.type == CONNACK)
				{
					...
					if (rc == MQTTASYNC_SUCCESS)
					{
						if (m->serverURIcount > 0)
							Log(TRACE_MAX, -1, "Connect succeeded to %s",
						...
						if (m->connect.onSuccess)
						{
							...
							Log(TRACE_MAX, -1, "Calling connect success for client %s", m->c->clientID);
							if (m->serverURIcount > 0)
						...
						if (m->connected)
						{
							Log(TRACE_MAX, -1, "Calling connected for client %s", m->c->clientID);
						...
						if (command->command.token == ((Suback*)pack)->msgId)
						{
							...
							if (sub->qoss->count == 1 && *(int*)(sub->qoss->first->content) == MQTT_BAD_SUBSCRIBE)
							{
								if (command->command.onFailure)
								{
									...
									Log(TRACE_MAX, -1, "Calling subscribe failure for client %s", m->c->clientID);
									(*(command->command.onFailure))(command->command.context, &data);
								}
							}
							else if (command->command.onSuccess)
							{
								...
								data.token = command->command.token;
								Log(TRACE_MAX, -1, "Calling subscribe success for client %s", m->c->clientID);
								(*(command->command.onSuccess))(command->command.context, &data);
								if (array)
									free(array);
							}
					...
					while (ListNextElement(m->responses, &current))
					{
						...
						if (command->command.token == ((Unsuback*)pack)->msgId)
						{
							...
							if (command->command.onSuccess)
							{
								...
								handleCalled = 1;
								Log(TRACE_MAX, -1, "Calling unsubscribe success for client %s", m->c->clientID);
...
void MQTTAsync_stop()
{
	...
	if (sendThread_state != STOPPED || receiveThread_state != STOPPED)
	{
		...
		Log(TRACE_MAX, -1, "Conn_count is %d", conn_count);
...
int MQTTAsync_deliverMessage(MQTTAsyncs* m, char* topicName, size_t topicLen, MQTTAsync_message* mm)
{
	...
	Log(TRACE_MAX, -1, "Calling messageArrived for client %s, queue depth %d",
			...
			if (SSLSocket_setSocketForSSL(&m->c->net, m->c->sslopts) != MQTTASYNC_SUCCESS)
			{
				if (m->c->session != NULL)
					if ((rc = SSL_set_session(m->c->net.ssl, m->c->session)) != 1)
						Log(TRACE_MAX, -1, "Failed to set SSL session with stored data, non critical");
			...
			else if (pack->header.bits.type == PUBACK || pack->header.bits.type == PUBCOMP)
			{
				...
				*rc = (pack->header.bits.type == PUBCOMP) ?
				...
				if (m)
				{
					...
					if (m->dc)
					{
						Log(TRACE_MAX, -1, "Calling deliveryComplete for client %s, msgid %d", m->c->clientID, msgid);
